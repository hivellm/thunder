...
- [ ] 1.6 Regenerate `Profile` constants + profile-pinning tests: rust/thunder-wire (profile.rs, tests/profiles.rs), typescript (profile.ts + test), python (profile.py + test — drop the PyYAML `off`→bool workaround once quoted), csharp (Profile.cs + test)
...
- [ ] 2.3 Run tests and confirm they pass — full gate green in all four languages (rust `cargo test -p thunder-wire`, ts `npm test`, py `pytest`, cs `dotnet test`) against the corrected registry
