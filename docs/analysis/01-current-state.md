...
1. **`Bytes` encoding.** Synap serializes `Bytes` as `Arc<[u8]>` through an `arc_bytes` adapter producing a MessagePack **array of integers** (`synap-protocol/src/synap_rpc/types.rs:35-48`); Nexus/Vectorizer produce MessagePack **bin** (Vectorizer's TS explicitly maps `Uint8Array` → bin "to match rmp-serde Bytes", `codec.ts:50-63`). Every Synap SDK decoder special-cases int-array Bytes. An int-array costs 1–2 bytes per element vs 1 for bin — Synap's form **forfeits the "4× smaller embeddings" claim** that the SDK contract mandates `Bytes` for (`Nexus/docs/specs/sdk-transport.md:178-181`).
